...
def to_sig_fig_float(
    ...
    '''
    assert isinstance(number, Union[int, float]), '`number` must be int or float.'
    assert isinstance(significant_figures, int), '`significant_figures` must be int.'
...
def seconds_to_short_time(
    ...
    Converts seconds to XdYhZm format.
        ...
        :return:        Time string.
    '''
    assert isinstance(secs, int), '`secs` must be int.'
